...
if [[ "$OPENEDX_RELEASE" == open-release/quince.* ]]; then
    EDXAPP_CELERY_PASSWORD=${EDXAPP_CELERY_PASSWORD:-celery}
    REDIS_PASSWORD=${REDIS_PASSWORD:-$EDXAPP_CELERY_PASSWORD}
    EXTRA_VARS="$EXTRA_VARS -e EDXAPP_CELERY_PASSWORD=$EDXAPP_CELERY_PASSWORD -e REDIS_PASSWORD=$REDIS_PASSWORD"
fi
